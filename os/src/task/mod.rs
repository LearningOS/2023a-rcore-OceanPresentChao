...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        if next_task.fst_start_time == 0 {
            next_task.fst_start_time = get_time_ms();
        }
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].fst_start_time == 0 {
                inner.tasks[next].fst_start_time = get_time_ms();
            }
    ...
    /// note syscall time
    pub fn note_syscall_time(&self, syscall_id: usize) {
    ...
    pub fn get_current_task_fst_time(&self) -> usize {
        let inner = self.inner.exclusive_access();
        let cur = inner.current_task;
        inner.tasks[cur].fst_start_time
    }
}
